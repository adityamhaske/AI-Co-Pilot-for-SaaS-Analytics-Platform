...
async def test_sse_endpoint_success(client, db_session):
    ...
    with patch("app.streaming.sse.client.messages.stream") as mock_stream:
        ...
        async def mock_events():
            # Yield a text event
            event = type(
                "Event", (), {"type": "text", "text": "The MRR is increasing."}
            )
            yield event

        mock_stream_context.__aenter__.return_value.text_stream = []
        mock_stream_context.__aenter__.return_value.__aiter__.side_effect = (
            lambda: mock_events()
        )
        ...
        mock_stream_context.__aenter__.return_value.get_final_message.return_value = (
            mock_final_message
        )
